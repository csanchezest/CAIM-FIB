...
def toTFIDF(client, index, file_id):
    ...
    # s'ha canviat tfidfw d'array a diccionari
    # tfidfw = []
    tfidfw = {}
    for (t, w),(_, df) in zip(file_tv, file_df):
        
        # implementacio del tfidf
        tf = w/max_freq
...
def print_term_weigth_vector(twv):
    ...
    # implementacio del print per pantalla dels valors del diccionari
    print(twv)
    pass
...
def normalize(tw):
    ...
    # implementacio de la normalitzacio, que en aquest cas requereix, primer, de normalitzar els valors i, segon, d'enllaçar-los de nou a les seves claus
    vals = list(tw.values())
...
def cosine_similarity(tw1, tw2):
    ...
    # recorrem ambdos diccionaris i els anem afegint els termes que no s'hi troben amb valor 0; com que es tracta de diccionaris, el cost temporal es idealment O(1)
    # despres d'afegir els termes que falten, reordenem els dos diccionaris; com que tindran els mateixos termes, els dos tindran el mateix ordre
    # finalment, podem aplicar l'operacio np.dot, que multiplicara adientment ambdos vectors de valors
    for key in tw2:
        if key not in tw1:
            tw1[key] = 0
    for key in tw1:
